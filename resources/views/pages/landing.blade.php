...
<div class="flex flex-col px-4 md:px-10 lg:px-14 mt-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        @foreach($news->skip(1) as $new)
            ...
            <img src="{{ asset('storage/' . $new->thumbnail) }}" alt="berita2" class="rounded-xl w-full md:max-h-48" style="width:250px; object-fit:cover">
    ...
    <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-4">
        ...
        <a href="{{ route('news.show', $newsd->slug) }}" class="block">
            <div
                ...
                <div class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal ml-2 mt-2 text-sm absolute">
                    {{ $newsd->newsCategory->title }}</div>
                <img src="{{asset('storage/' . $newsd->thumbnail) }}" alt="" class="w-full rounded-xl mb-3" style="height: 200px; object-fit:cover">
                <p class="font-bold text-base mb-1">{{ $newsd->title}}</p>
                <p class="text-slate-400">{{\Carbon\Carbon::parse($featured->created_at)->format('d F Y')}} </p>
            </div>
        </a>
        @endforeach
