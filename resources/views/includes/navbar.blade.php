      ...
      <div class="flex gap-10 w-full">
          ...
          <div class="flex items-center justify-between w-full lg:w-auto">
              ...
              <a href="{{ route('landing') }}" class="flex items-center gap-2">
                  <div class="flex items-center gap-2">
                      <img src="{{ asset('assets/img/Logo.png') }}" alt="Logo" class="w-20 lg:w-10">
                      <p class="text-lg lg:text-xl font-bold">News Portal Infrastructure</p>
                  </div>
              </a>
          ...
          <div id="menu"
              ...
              <ul
                  ...
                  <li><a href="{{ route('landing') }}" class="{{ request()->routeIs('landing') ? 'text-primary' : 'hover:text-black' }} hover:text-primary">Beranda</a></li>
                  @foreach(\App\Models\NewsCategory::all() as $category)
                  <li><a href="{{ route('news.category', $category->slug) }}" class="{{ request()->routeIs('news.category') && request()->route('slug') === $category->slug ? 'text-primary' : 'hover:text-black' }} hover:text-primary">{{ $category->title }}</a></li>
                  @endforeach
              </ul>
          </div>
      </div>
          ...
          <div class="relative w-full lg:w-auto">
              <form action="{{ route('news.index') }}" method="GET">
                  <input type="text" name="search" placeholder="Cari Berita"
                      class="border border-slate-300 rounded-full px-4 py-2 pl-8 w-full text-sm font-normal lg:w-auto focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
